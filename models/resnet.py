...
class ResNet(nn.Module):
    def __init__(self, block, layers, dilation=[1,1,1,1], bn_momentum=0.0003, is_fpn=False):
        ...
        self.conv1 = nn.Sequential(
            ...
            BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, self.inplanes, 3, stride=1, padding=1, bias=False),
        )
...
def get_resnet101(dilation=[1,1,1,1], bn_momentum=0.0003, is_fpn=False, is_pretrained=True):
    model = ResNet(Bottleneck, [3, 4, 23, 3], dilation=dilation, bn_momentum=bn_momentum, is_fpn=is_fpn)
    if is_pretrained:
        model_url = 'D:/Hongpeng/Xiong/Projects/my_impl/models/pretrained/resnet101.pth'
