...
class Head(nn.Module):
    def __init__(self, classify_classes, norm_layer=nn.BatchNorm2d, bn_momentum=0.0003,rep_head=False):
        ...
        self.classify = nn.Sequential(nn.Conv2d(304, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       norm_layer(256, momentum=bn_momentum),
                                       nn.ReLU(),
                                       nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       norm_layer(256, momentum=bn_momentum),
                                       nn.ReLU(),
                                       nn.Conv2d(in_channels=256, out_channels=self.classify_classes, kernel_size=1,
                                        stride=1, padding=0, dilation=1, bias=True)
                                       )

        if self.rep_head:
            self.represente= nn.Sequential(nn.Conv2d(304, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                            norm_layer(256, momentum=bn_momentum),
                                            nn.ReLU(),
                                            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                            norm_layer(256, momentum=bn_momentum),
    ...
    def forward(self, f_list):
        ...
        res = {'pred':pred}
        if self.rep_head:
            res['rep'] = self.represente(f)
        return res

class DeepLabV3plus(nn.Module):
    def __init__(self, class_num, bn_momentum=0.01):
        super(DeepLabV3plus, self).__init__()
        self.Resnet101 = get_resnet101(dilation=[1, 1, 1, 2], bn_momentum=bn_momentum, is_fpn=True,is_pretrained=True)
        self.head = Head(class_num, norm_layer=nn.BatchNorm2d,rep_head=True)

    def forward(self, input):
        x = self.Resnet101(input)
        output = self.head(x)

        #output['pred']= F.interpolate(output['pred'], size=input.size()[2:4], mode='bilinear', align_corners=True)
        return output

def main():
    num_classes = 4
    in_batch, inchannel, in_h, in_w = 4, 3, 288, 288
