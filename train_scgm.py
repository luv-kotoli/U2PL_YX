...
from utils import set_random_seed, AverageMeter, cal_category_confidence,init_log,load_state,label_onehot,intersectionAndUnion,dice_coef_2
...
parser.add_argument('--dataset',default='scgm')
parser.add_argument('--cuda',default=True)
parser.add_argument('--num_classes',default=2,type=int,help='4 classes for MNMS dataset and 2 classes for SCGM dataset')
...
parser.add_argument('--use_contra_loss',default=False)
...
def main():
    ...
    sup_loss_fn =  get_criterion(criterion=None,ignore_index=255,thresh=0.7,)
    ...
    if args.use_pretrained==True :
        lastest_model = os.path.join(save_path, 'ckpt_scgm_{}.pth'.format(args.domain))
    ...
    for epoch in range(last_epoch, args.epoch):
        ...
        if args.is_eval:
            ...
            if epoch < super_only_epoch:
                prec,dice_mean = validate(model, val_loader, epoch, logger,num_classes=args.num_classes,ignore_label=255)
            else:
                prec,dice_mean = validate(model_teacher, val_loader, epoch, logger,num_classes=args.num_classes,ignore_label=255)
            ...
            if prec > best_prec:
                ...
                best_dice = dice_mean
                torch.save(
                    state, os.path.join(save_path, "ckpt_scgm_best_{}_no_con.pth".format(args.domain))
                )

            torch.save(state, os.path.join(save_path, "ckpt_scgm_{}_no_con.pth".format(args.domain)))

            logger.info(
                "\033[31m * Currently, the best val result is: {:.2f}, dice is {:.4f}\033[0m".format(
                    best_prec * 100, best_dice*100
                )
            )
            tb_logger.add_scalar("mIoU val", prec, epoch)
            tb_logger.add_scalar('dice_mean',dice_mean,epoch)
            #log_images([x[0] for x in log_imgs],tb_logger,epoch,image_name='image')
            #log_images([x[1] for x in log_imgs],tb_logger,epoch,is_masks=True,image_name='mask')
            #log_images([x[2] for x in log_imgs],tb_logger,epoch,is_masks=True,image_name='result')
def test():
    global args
    dice_score = 0
    output_path = './output/scgm/vendor{}/'.format(args.domain)
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    save_path = './checkpoints'
    class_num=args.num_classes
    current_time = time.get_clock_info
    lastest_model = os.path.join(save_path, 'ckpt_scgm_best_{}.pth'.format(args.domain))
    ...
    for i,batch in tqdm(enumerate(test_loader)):
        ...
        output_2 = label_onehot(output_temp,args.num_classes)
        target_2 = masks_onehot

        dice_temp = 0
        dice_temp+= dice_coef_2(output_2[:,1,:,:],target_2[:,1,:,:])
    ...
    **kwargs
    ...
    for step in range(len(loader_u)):
        ...
        if epoch < kwargs['super_only_epoch']:
            ...
            #print(pred_l_large.shape,label_l.shape)
            # supervised loss
        ...
        if epoch >= kwargs.get("super_only_epoch", 1):
            with torch.no_grad():
                ema_decay = min(
                    ...
                    / (
                        i_iter
                        - len(loader_l) * kwargs['super_only_epoch']
                        + 1
                    ),
                    ema_decay_origin,
                )
