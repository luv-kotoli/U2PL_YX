...
def get_optimizer(parms, **kwargs):
    """
    Get the optimizer
    """
    optim_type = kwargs["type"]
    optim_kwargs = kwargs["opt_args"]
    if optim_type == "SGD":
...
def get_scheduler(epochs,lr_mode,lr_args, len_data, optimizer, start_epoch=0, use_iteration=False):
    epochs = epochs if not use_iteration else 1
